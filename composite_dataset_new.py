...
if __name__ == '__main__':
  ...
  for case_id in image_id_list:  
    ...
    if (user_count > 0):#user is available
      ...
      for tmp_algorithm in objects.distinct("algorithm",{"provenance.image.case_id":case_id,
        ...
        tmp_prefixs_algorithm[1]=tmp_algorithm;        
  ...
  with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor0:  
  ...
  for user_case_id in  process_list:
    ...
    for index1 in range (0,len(prefixs_algorithm)):
      ...
      with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor1:
        for tmp_title_polygon in tmp_title_polygon_list:
          executor1.submit(process_one_tile, title_index,algorithm,tmp_title_polygon,algorithm_folder,out_algorithm_folder);        
          title_index+=1;
          
      print "--- copy composite_results to the storage node in cluster --- ";  
      remote_composite_results_path ='/data/shared/bwang/composite_results/'; 
      ...
      has_remote_storage_folder=False;
      ...
      if(return_code=='False'):#remote folder does NOT exist, then create it      
        while (not has_remote_storage_folder):  
          subprocess.Popen(["ssh", "%s" % storage_node, command2],shell=False,stdout=subprocess.PIPE,stderr=subprocess.PIPE);           
          return_code = subprocess.Popen(["ssh", "%s" % storage_node, command1],shell=False,stdout=subprocess.PIPE,stderr=subprocess.PIPE);
          result = return_code.stdout.readlines();  
          return_code=result[0].replace('\n','');     
          if(return_code=='True'):#remote folder exist, continue
            has_remote_storage_folder=True;             
  ...
  remote_composite_results_path ='/data/shared/bwang/composite_results/'; 
  with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor2:
